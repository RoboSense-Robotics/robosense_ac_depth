...
#include "infer_base/common_utils/block_queue.hpp"
#include "infer_base/common_utils/log.hpp"
#include "infer_base/common_utils/types.hpp"
                         ...
                         const InnerBlock_t                           &pipeline_block)
  {
    ...
    while (!pipeline_close_flag_)
    {
      ...
      try
      {
        ...
        float dt = 0.001 * (float)std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        // LOG_INFO("Time_Cost_Block_Name, %s, cost(ms), %f",
        //           pipeline_block.GetName().c_str(),
        //           dt);
