...
class InferenceStereo : public BaseStereoMatchingModel {
public:
  InferenceStereo(const std::shared_ptr<BaseInferCore>    &infer_core,
...
InferenceStereo::InferenceStereo(const std::shared_ptr<BaseInferCore>    &infer_core,
                         const std::shared_ptr<IImageProcessing> &preprocess_block,
                         const int                                input_height,
                         const int                                input_width,
                         const std::vector<std::string>          &input_blobs_name,
      ...
      output_blobs_name_(output_blobs_name)
{
  ...
  {
    LOG_ERROR("[InferenceStereo] Infer core should has {%d} blobs, but got {%d} blobs!",
              input_blobs_name_.size() + output_blobs_name_.size(), blobs_tensor->Size());
    throw std::runtime_error("[InferenceStereo] Got invalid input arguments!!");
  }
...
bool InferenceStereo::PreProcess(std::shared_ptr<IPipelinePackage> _package)
{
  auto package = std::dynamic_pointer_cast<StereoPipelinePackage>(_package);
  CHECK_STATE(package != nullptr,
              "[InferenceStereo] PreProcess the `_package` instance does not belong to "
...
bool InferenceStereo::PostProcess(std::shared_ptr<IPipelinePackage> _package)
{
  auto package = std::dynamic_pointer_cast<StereoPipelinePackage>(_package);
  CHECK_STATE(package != nullptr,
              "[InferenceStereo] PostProcess the `_package` instance does not belong to "
              "`DetectionPipelinePackage`");
  ...
  CHECK_STATE(output_disp != nullptr,
              "[InferenceStereo] `PostProcess` Got invalid output disp ptr !!!");
...
std::shared_ptr<BaseStereoMatchingModel> CreateInferenceStereoModel(
    ...
    const std::vector<std::string>          &output_blobs_name)
{
  return std::make_shared<InferenceStereo>(infer_core, preprocess_block, input_height, input_width,
