    ...
    std::unordered_map<std::string, std::vector<uint64_t>> &blobs_shape)
{
  ...
  for (int i = 0; i < blob_number; ++i)
  {
    ...
    if (resolve_blob_shape)
    {
      ...
      {
        // Check whether dynamic shapes exist; automatic resolution does not support dynamic shapes
        if (dim.d[j] <= 0)
        {
...
bool TrtInferCore::PostProcess(std::shared_ptr<IPipelinePackage> pipeline_unit)
{
  ...
  for (int i = 0; i < blob_number; ++i)
  {
    ...
    LOG_DEBUG("PostProcess tensor: %s\n", s_blob_name.c_str());
    if (tensor->current_location_ == DataLocation::HOST)
    {
...
{
  if (!FileSuffixCheck(model_path, ".plan"))
  {
    throw std::invalid_argument("Trt infer core expects file end with `.plan`. But got " +
                                model_path + " instead");
  }
