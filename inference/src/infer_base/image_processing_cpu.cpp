...
private:
  ...
  void FlipChannelsWithoutNorm(const cv::Mat &image, float *dst_ptr, bool flip);
  void TransposeAndFilpWithNorm(const cv::Mat &image, float *dst_ptr, bool flip);
  void TransposeAndFilpWithoutNorm(const cv::Mat &image, float *dst_ptr, bool flip);
                                           ...
                                           int                                 dst_width)
{
  ...
  if (!do_transpose_)
  {
    ...
    {
      FlipChannelsWithoutNorm(dst_image, dst_ptr,
  ...
  } else
  {
    ...
    {
      TransposeAndFilpWithoutNorm(dst_image, dst_ptr,
                                  image_data_info.format == ImageDataFormat::BGR);            
    }
  }
                                                          ...
                                                          bool           flip)
{
  ...
  for (int r = 0; r < rows; ++r)
  {
    ...
    {
      size_t idx       = offset + c * 3;
      dst_ptr[idx + 0] = (float)image.data[idx + r_idx];
      dst_ptr[idx + 1] = (float)image.data[idx + g_idx];
      dst_ptr[idx + 2] = (float)image.data[idx + b_idx];
    }
  }
}
  ...
  for (int r = 0; r < rows; ++r)
  {
    ...
    for (int c = 0; c < cols; ++c)
    {
      ...
      dst_ptr[offset_idx + 0 * single_channel_pixel_size] =
          (pixel_ptr[r_idx] / 255.0 - mean_[r_idx]) / val_[r_idx];

      dst_ptr[offset_idx + 1 * single_channel_pixel_size] =
          (pixel_ptr[g_idx] / 255.0 - mean_[g_idx]) / val_[g_idx];

      dst_ptr[offset_idx + 2 * single_channel_pixel_size] =
          (pixel_ptr[b_idx] / 255.0 - mean_[b_idx]) / val_[b_idx];
                                                              ...
                                                              bool           flip)
{
  ...
  for (int r = 0; r < rows; ++r)
  {
    ...
    for (int c = 0; c < cols; ++c)
    {
      ...
      dst_ptr[offset_idx + 0 * single_channel_pixel_size] = (float)pixel_ptr[r_idx];
      dst_ptr[offset_idx + 1 * single_channel_pixel_size] = (float)pixel_ptr[g_idx];
      dst_ptr[offset_idx + 2 * single_channel_pixel_size] = (float)pixel_ptr[b_idx];
